...
public class RedBlackTree<T extends Comparable<T>> {
    ...
    public boolean add(T value){ // Метод добавления элемента (публичная версия).
        if(root == null){
            root = new Node(value);
    ...
    private Node addNode(Node node, T value) { // Добавление элемента.
    ...
    private Node rebalance(Node node) { // Баласировка
        Node result = node;
    ...
    private class Node{ // Класс указателей (ссылка)
        T value;
        Node left; 
        Node right; 
        Color color;
